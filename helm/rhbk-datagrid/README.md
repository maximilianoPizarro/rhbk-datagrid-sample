...
![OpenShift Topology View](https://raw.githubusercontent.com/maximilianoPizarro/rhbk-datagrid-sample/main/docs/datagrid-keycloak-topology.png)
...
| **Data Grid (x2)** | Two Infinispan server instances forming their own cluster via **JGroups DNS_PING** (headless Kubernetes service). They provide a remote distributed cache for Keycloak sessions. |
| **PostgreSQL (x1)** | Single-instance database storing Keycloak data and serving as the registry for JGroups `JDBC_PING2` node discovery. |
